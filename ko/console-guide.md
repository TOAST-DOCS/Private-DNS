...
2. **Private DNS Zone 이름**과 **설명**을 입력하고, VPC를 선택하여 **확인** 버튼을 클릭합니다.  
    ...
    - 생성이 완료되면 Private DNS 서비스를 이용하기 위해 연결된 VPC의 인스턴스들은 네임서버를 프라이빗 DNS 포트 IP로 변경해야 합니다.
      ...
      - 재시작없이 네임서버를 변경하려면, 인스턴스에 접속해 네임서버를 프라이빗 DNS 포트 IP로 지정해야 합니다. 운영체제별로 네임서버를 설정하는 방법에 따라 다릅니다. 
        > [참고] 운영체제별 네임서버 변경 방법
        > - centos 7.9 및 ubuntu 22.04
        >   - root 권한을 통해 /etc/resolve.conf 파일을 변경해야 합니다.
        >   - /etc/resolv.conf 파일에 기존의 네임서버 설정을 주석 처리 혹은 제거한 후, 네임서버 설정을 새로 추가해야 합니다. 파일 변경 후 바로 네임서버 변경이 적용됩니다.
        >     - ```diff
        >       nameserver {프라이빗 DNS 포트 IP}
        >       ```
        >       - 프라이빗 DNS 포트 IP는 **네트워크** > **Network Interface**에서 장치가 **PRIVATE_DNS_SERVER**이고, 설정한 VPC에 속해있는 네트워크 인터페이스의 사설 IP입니다.
