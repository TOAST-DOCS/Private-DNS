...
- In **Private DNS Zone Name**, enter the domain or subdomain you own as a fully qualified domain name (FQDN).
    - The **Private DNS Zone Name** must be unique within the connected VPC.
    ...
    - Private DNS Zone can be connected to a minimum of one VPC, and the maximum number of connections is unlimited.
    - Once created, instances in the connected VPC must change their nameservers to the private DNS port IPs in order to use the Private DNS service.
      - Restarting the instance sets the instance's nameservers to the private DNS port IPs.
    ...
    - You can view the record sets created by default in [Manage Record Sets](#manage-record-sets).
